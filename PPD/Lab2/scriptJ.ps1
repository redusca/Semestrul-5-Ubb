...
$param1 = $args[0] # Nume clasa Java (fara .class)
$param2 = $args[1] # No of threads
$param3 = $args[2] # No of runs
$param4 = "input/" + $args[3] # Nume fisier input
$param5 = $args[4] # Caz 1 - secvential , 2 - paralele

$Matrice = $args[5] 

$output_file = $args[6] # Nume fisier output
...
for ($i = 0; $i -lt $param3; $i++){
    ...
    $a = (java -Xms4G -Xmx6G $param1 $param4 $param2 $param5 2>&1 | Select-Object -Last 1)
    Write-Host $a
    $suma += $a
    Write-Host ""
}

# Validate output if not sequential (mode 2)
if ($param5 -ne 1) {
    Write-Host ""
    Write-Host "Validating parallel output against sequential baseline..."
    
    $parallel_dir = "output/p"
    $mode_name = "Parallel"
...
# Append
Add-Content outJ.csv "$($tip_matrice),$($t),$($media) ns"
