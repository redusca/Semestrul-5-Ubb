...
def run_test(exe_name, num_threads, num_runs, input_file, mode, matrix_type, output_file):
    """
...
def main():
    ...
    matrices = [
        ...
        (2, "matrix_5_1000_1000.txt", 3, 1000, 1000, [2, 4, 8, 16]), # N=M=1000, k=3, p=2,4,8,16
        (3, "matrix_5_10000_10000.txt", 3, 10000, 10000, [2, 4, 8, 16]) # N=10000, M=10000, k=3, p=2,4,8,16
    ]
    
    # Execution modes: 1=sequential, 2=parallel
    modes = [1, 2]
    
    print("Starting comprehensive test suite for C++")
    ...
    for matrix_type, input_file, k, n, m, thread_counts in matrices:
        for mode in modes:
            if mode == 1:
                ...
                output_file = f"outputC_{k}_{n}_{m}.txt"
                run_test(exe_name, 1, num_runs, input_file, mode, matrix_type, output_file)
            else:
                # Parallel mode - iterate through different thread counts
                for num_threads in thread_counts:
                    current_test += 1
                    print(f"\n\nProgress: Test {current_test}/{total_tests}")
                    output_file = f"outputC_{k}_{n}_{m}.txt"
                    run_test(exe_name, num_threads, num_runs, input_file, mode, matrix_type, output_file)
